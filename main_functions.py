...
def open_files(father_file_path,middle_path,file_name,check='',ip='',input_days=3):
    #打开文件
    ...
    if middle_path == 'conf':
        ...
        #拼接  sql
        tmp_sql = file.read()
        full_sql = tmp_sql%(check,input_days,ip)
        return full_sql
    file.close()

def write_line(target_conn,line_list,ip,table,m):
    '''写查询结果到excel'''
    sql_result = target_conn.fetchall()
    # m = m + len(sql_result)
    if len(sql_result) > 0:
        for each_result in sql_result:
            new_sql_result = line_list + list(each_result)
            # print new_sql_result
            for i in range(len(new_sql_result)):
                style = properties(22)
                table.write(m, i, new_sql_result[i], style)
                style = properties(5)
                table.write(m,27,new_sql_result[i-2],style)
                style = properties(6)
                table.write(m, 30, new_sql_result[i - 1], style)
                style = properties(7)
                table.write(m, 33, new_sql_result[i], style)
            m = m + 1
        print (ip+" done !!")
        return m
    else:
        print (ip +  " has no results !!")
        return m

def del_null_excel(father_file_path,type=''):
    # del 用的py2excel，删掉没有top sql的excel。 获取需要遍历的excel
    # move 用到 ex2oracle 中，加载完忽略sql后，将excel 移动到 excel_bak目录
    ...
    for excel in sql_list:
        ...
        if type == 'del':
            ...
            pass
            # shutil.move(full_excel,target_excel)
